import { BotScreens, WalletScreens } from '../screens';
import { BotKeyboards, WalletKeyboards, GeneralKeyboards } from '../keyboards';
...
export class WalletHandler {
	static async showWalletMain(ctx: BotContext) {
		...
		const screen = WalletScreens.getWalletMainScreen(walletCount);
		const keyboard = WalletKeyboards.getWalletMainKeyboard();
		try {
	...
	static async showWalletList(ctx: BotContext, page = 0) {
		...
		const screen = WalletScreens.getWalletListScreen(pageWallets, page, totalPages);
		const keyboard = WalletKeyboards.getWalletListKeyboard(pageWallets, page, totalPages);
		try {
			await ctx.editMessageText(BotScreens.formatScreen(screen), {
				parse_mode: 'Markdown',
				reply_markup: keyboard.reply_markup
	...
	static async showWalletDetail(ctx: BotContext, walletId: string) {
		...
		const screen = WalletScreens.getWalletDetailScreen(wallet);
		const keyboard = WalletKeyboards.getWalletDetailKeyboard(walletId);
		await ctx.editMessageText(BotScreens.formatScreen(screen), {
			parse_mode: 'Markdown',
			reply_markup: keyboard.reply_markup
		});
	...
	static async generateWallet(ctx: BotContext) {
		...
		if (result.success && result.data) {
			await ctx.reply(`✅ New wallet generated: \`${result.data.address}\``, {
				parse_mode: 'Markdown',
				reply_markup: GeneralKeyboards.getClosableKeyboard().reply_markup
			});
		} else {
